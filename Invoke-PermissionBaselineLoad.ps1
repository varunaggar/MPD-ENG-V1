...
try {
    foreach ($mailbox in $allMailboxes) {
        ...
        # Fix C: Refresh EXO session periodically to prevent timeout during multi-hour run
        if ($phase3Processed % 1000 -eq 0 -and $phase3Processed -gt 0) {
            Write-LogInfo "Refreshing EXO session (every 1000 mailboxes)..."
            Disconnect-ExoSession
            Connect-ExoSession -Config $Config
        }
    }
...
try {
    foreach ($mailbox in $allMailboxes) {
        ...
        try {
            ...
            # Fix D: Mark THIS mailbox as baselined immediately after folder perms commit.
            # If the baseline crashes mid-Phase-5, only completed mailboxes are marked.
            # The delta's Phase 0 will mini-baseline any remaining NULL mailboxes.
...
try {
    ...
    # Fix A: Seed the delta timestamp too — ensures Invoke-PermissionDeltaSync
    # picks up from where the baseline finished, with no gap window.
    Save-DeltaToken -TokenName "permissions_delta_timestamp"    -TokenValue $savedTimestamp
    Write-LogInfo "Baseline + delta timestamps saved: $savedTimestamp"
}
catch {
    $counters.Errors++
